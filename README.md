...
- The thermodynamic and transport properties of this `c` soot pseudo-species are based on the [`CSOLID` entry from the CRECK Modeling Lab mechanism](https://github.com/CRECKMODELING/Kinetic-Mechanisms/tree/main/Gas-Phase/Diesel-Biodiesel/Soot-NOx/TOT_HT_SOOT_452_24041): CRECK NASA-7 heat capacity for 300-3500 K, `M = 12.011 kg/kmol`, `sigma = 12.91 Angstrom`, and `epsilon/k = 2302 K`. Fluent evaluates soot viscosity, thermal conductivity, and mixture diffusion with kinetic theory. The assumed soot density `1500 kg/m3` follows [Pejpichestakul et al. (2019)](https://doi.org/10.1016/j.combustflame.2019.04.001).
- Kinetic-theory mixture diffusion requires Lennard-Jones parameters for every participating species. The compact six-step files therefore also include the same existing tar transport parameters used in `Glarborg148.scm` and `Li37.scm`; the tar thermodynamic definition itself is unchanged.
